return {
  ...
  opts = {
    ...
    styles = {
      terminal = {
        ...
        stack = true,
        wo = {
          winhighlight = "Normal:Normal,NormalNC:Normal,WinBar:Normal,WinBarNC:Normal,SignColumn:Normal,CursorLine:Normal",
          cursorline = false,
          number = false,
  ...
  keys = {
    ...
    { "<leader>zd", function()
      ...
      if dim.enabled then
        dim.disable()
        require("util").notify("Dim Off")
      else
        dim.enable()
        require("util").notify("Dim On")
      end
